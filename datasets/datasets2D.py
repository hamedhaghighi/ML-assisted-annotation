...
def write_annotations(ind, data_dict, pre_annotation, pre_label_dir, labels, opt):
    ...
    if opt.data_format == "kitti":
            ...
            if pre_annotation is not None:
                ...
                for pre_ann in pre_annotation:
                    ...
                    c, x1, y1, x2, y2 = unpack_pre_annotation(
                        pre_ann, pad_x, pad_y, unpad_h, unpad_w, h, w, labels
                    )
                    x1, y1, x2, y2 = (
                        round(x1, 2),
                        round(y1, 2),
        ...
        attr_dict = {
            "text": [{"view": "Front"}],
            "boolean": [],
            "others": [{"score": 0}, {"person_id": 0}, {"bbox_id": 0}, {"lane_id": 0}],
        }
        if pre_annotation is not None:
            pre_annotation = pre_annotation.cpu().numpy()
            for i, pre_ann in enumerate(pre_annotation):
                ...
                bbox = [
                    {"name": c, "val": [x1, y1, width, height], "attributes": attr_dict}
                ]
...
def collate_fn(list_data):
    ...
    rt_data_dict = {
        k: torch.stack(v, dim=0) if torch.is_tensor(v[0]) else v
        for k, v in rt_data_dict.items()
    }
    return rt_data_dict
    ...
    def __getitem__(self, index):
        ...
        for ann in object_anns:
            if idx < self.max_objects:
                ...
                c = category_to_class(category_name["name"], self.filter_labels)
    ...
    def __init__(
        ...
        if self.IMG_EXT == "":
            if img_root_dir is None:
                self.IMG_EXT = self.images_path[0].split(".")[-1]
            else:
                path, _ = get_image_labels_path(img_root_dir, data_format)
                self.IMG_EXT = path[0].split(".")[-1]
    ...
    def __getitem__(self, index):
        ...
        if self.img_root_dir is not None:
            if self.data_format == "kitti":
                img_name = label_path.split(self.sep)[-1].replace("txt", self.IMG_EXT)

                im_path = os.path.join(self.img_root_dir, "image_2", img_name)
            elif self.data_format == "openlabel":
                label_file_name = label_path[list(label_path.keys())[0]][
                    "file"
                ].replace("txt", self.IMG_EXT)
                ...
                for line in f.readlines():
                    ...
                    if idx < self.max_objects:
                        ...
                        if c != -1:
                            labels[idx, 0] = c
                            bbox = [
                                b * (ratio if ratio is not None else 1) for b in bbox
                            ]
                            labels[idx, 1] = (
                                (bbox[0] + bbox[2] + 2 * pad[1][0]) / 2
                            ) / padded_w
        ...
        filled_labels = torch.from_numpy(labels)
        rt_data_dict = dict(
            im_path=im_path, input_img=input_img, filled_labels=filled_labels
        )
        return rt_data_dict
