...
def points_camera2image(points, P2):
    ...
    return: shape=(N, 8, 2)
    """
    extended_points = np.pad(
        points, ((0, 0), (0, 0), (0, 1)), "constant", constant_values=1.0
    )  # (n, 8, 4)
...
def group_plane_equation(bbox_group_rectangle_vertexs):
    ...
    vectors = (
        bbox_group_rectangle_vertexs[:, :, :2] - bbox_group_rectangle_vertexs[:, :, 1:3]
    )
    normal_vectors = np.cross(vectors[:, :, 0], vectors[:, :, 1])  # (n, 6, 3)
    normal_d = np.einsum(
        "ijk,ijk->ij", bbox_group_rectangle_vertexs[:, :, 0], normal_vectors
    )  # (n, 6)
    plane_equation_params = np.concatenate(
        [normal_vectors, -normal_d[:, :, None]], axis=-1
    )
...
def remove_pts_in_bboxes(points, bboxes, rm=True):
    ...
    bbox_group_rectangle_vertexs = group_rectangle_vertexs(
        bboxes_corners
    )  # (n, 6, 4, 3)
...
def iou3d(bboxes1, bboxes2):
    ...
    bboxes1_top, bboxes2_top = (
        bboxes1[:, 2] + bboxes1[:, 5],
        bboxes2[:, 2] + bboxes2[:, 5],
    )  # (n, ), (m, )
    bboxes_bottom = torch.maximum(
        bboxes1_bottom[:, None], bboxes2_bottom[None, :]
    )  # (n, m)
...
def points_in_bboxes_v2(
    points, r0_rect, tr_velo_to_cam, dimensions, location, rotation_y, name
):
    """
    points: shape=(N, 4)
    tr_velo_to_cam: shape=(4, 4)
    ...
    indices = points_in_bboxes(
        points[:, :3], frustum_surfaces
    )  # (N, n), N is points num, n is bboxes number
    return indices, n_total_bbox, n_valid_bbox, bboxes_lidar, name


def get_points_num_in_bbox(
    points, r0_rect, tr_velo_to_cam, dimensions, location, rotation_y, name
):
    """
    points: shape=(N, 4)
    ...
    return: shape=(n, )
    """
    indices, n_total_bbox, n_valid_bbox, bboxes_lidar, name = points_in_bboxes_v2(
        points=points,
        r0_rect=r0_rect,
...
def remove_outside_points(points, r0_rect, tr_velo_to_cam, P2, image_shape):
    ...
    frustum = points_camera2lidar(
        frustum.T[None, ...], tr_velo_to_cam, r0_rect
    )  # (1, 8, 3)
...
def read_calib(file_path, extend_matrix=True):
    ...
    P0 = np.array([item for item in lines[0].split(" ")[1:]], dtype="float").reshape(
        3, 4
    )
    P1 = np.array([item for item in lines[1].split(" ")[1:]], dtype="float").reshape(
        3, 4
    )
    P2 = np.array([item for item in lines[2].split(" ")[1:]], dtype="float").reshape(
        3, 4
    )
    P3 = np.array([item for item in lines[3].split(" ")[1:]], dtype="float").reshape(
        3, 4
    )

    R0_rect = np.array(
        [item for item in lines[4].split(" ")[1:]], dtype="float"
    ).reshape(3, 3)
    Tr_velo_to_cam = np.array(
        [item for item in lines[5].split(" ")[1:]], dtype="float"
    ).reshape(3, 4)
    Tr_imu_to_velo = np.array(
        [item for item in lines[6].split(" ")[1:]], dtype="float"
    ).reshape(3, 4)
