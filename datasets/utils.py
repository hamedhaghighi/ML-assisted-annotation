...
def read_calib(file_path, extend_matrix=True):
    ...
    Tr_velo_to_cam = np.array([item for item in lines[5].split(' ')[1:]], dtype='float').reshape(3, 4)
    Tr_imu_to_velo = np.array([item for item in lines[6].split(' ')[1:]], dtype='float').reshape(3, 4)
...
def read_label(file_path):
    ...
    annotation['dimensions'] = np.array([line[8:11] for line in lines], dtype='float')[:, [2, 0, 1]] # hwl -> camera coordinates (lhw)
