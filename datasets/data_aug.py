...
def dbsample(CLASSES, data_root, data_dict, db_sampler, sample_groups):
    ...
    for name, v in sample_groups.items():
        ...
        sampled_cls_bboxes = np.array(
            [item["box3d_lidar"] for item in sampled_cls_list], dtype=np.float32
        )
        ...
        coll_query_matrix = np.concatenate(
            [avoid_coll_boxes_bv_corners, sampled_cls_bboxes_bv_corners], axis=0
        )
...
def object_noise_core(
    pts, gt_bboxes_3d, bev_corners, trans_vec, rot_angle, rot_mat, masks
):
    """
    pts: (N, 4)
    ...
    for i in range(n_bbox):
        for j in range(num_try):
            cur_bbox = bev_corners[i] - np.expand_dims(
                gt_bboxes_3d[i, :2], 0
            )  # (4, 2) - (1, 2) -> (4, 2)
            ...
            if coll_mat.any():
                ...
                bev_corners[i] = (
                    cur_bbox  # update the bev_corners when adding noise succseefully.
                )
                succ_mask[i] = j
                break
...
def object_noise(data_dict, num_try, translation_std, rot_range):
    ...
    trans_vec = np.random.normal(
        scale=translation_std, size=(n_bbox, num_try, 3)
    ).astype(np.float32)
    rot_angle = np.random.uniform(
        rot_range[0], rot_range[1], size=(n_bbox, num_try)
    ).astype(np.float32)
    ...
    masks = remove_pts_in_bboxes(
        pts, gt_bboxes_3d, rm=False
    )  # identify which point should be added noise
...
def random_flip(data_dict, random_flip_ratio):
    ...
    return: data_dict
    """
    random_flip_state = np.random.choice(
        [True, False], p=[random_flip_ratio, 1 - random_flip_ratio]
    )
...
def global_rot_scale_trans(data_dict, rot_range, scale_ratio_range, translation_std):
    ...
    rot_cos, rot_sin = np.cos(rot_angle), np.sin(rot_angle)
    # in fact, - rot_angle
    rot_mat = np.array([[rot_cos, rot_sin], [-rot_sin, rot_cos]])  # (2, 2)
    # 1.1 bbox rotation
...
def data_augment(CLASSES, data_root, data_dict, data_aug_config):
    ...
    data_dict = global_rot_scale_trans(
        data_dict, rot_range, scale_ratio_range, translation_std
    )
