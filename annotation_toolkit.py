...
import requests
from cvat_sdk.api_client import ApiClient, Configuration, apis, exceptions, models
...
class AnnotationToolkit:
    def __init__(self, opt, is_gui, text_browser_signal=None):
        ...
        if self.use_api and self.tool_name == "cvat":
            if is_gui:
                self.print_msg(
                    "Please enter your CVAT account credential on the opened window."
                )
    ...
    def try_func(self, fn, is_upload_annotation=False, **kwargs):
        ...
        self.print_msg(
            "Couldn't make the api call after maximum tries. Please try again."
        )
        exit(1)
            ...
            if self.tool_name == "cvat":
                with ApiClient(self.configuration) as api_client:
                    ...
                    if len(data.results) > 0:
                        ...
                        self.print_msg(f"Creating annotation task on CVAT ...")
                        task, _ = self.try_func(
                            api_client.tasks_api.create, task_write_request=task_spec
                        )
                        self.task_id = task.id
                        self.print_msg(
                            f"Annotation task with id:{self.task_id} is created on CVAT."
                        )
                        ...
                        task_data = models.DataRequest(
                            ...
                            client_files=[
                                ...
                                for img_path in list(
                                    glob(os.path.join(root_files, "*.jpeg"))
                                )
                            ],
                        )
                        self.try_func(
                        ...
                        if self.is_gui:
                            if ret == QMessageBox.Yes:
                                while True:
                                    ret = show_msgbox(
                                        self,
                                        "Have you saved the project on CVAT?",
                                        "yes/no",
                                        "info",
                                        self.is_gui,
                                    )
    ...
    def get_messages(self, key):
        ...
        if self.tool_name == "cvat":
            ...
            api_msg = f"ML-ADA is going to pop up CVAT app on the chrome. Please use the following guidelines to manually refine the annotations: {br} \
