...
# --- Instruction dialog for displaying HTML messages in the GUI ---
class InstructionUI(QDialog, Ui_Dialog):
    def __init__(self, parent, message):
        super().__init__()

        self.setupUi(self)
        # Display the message as HTML in the instruction dialog
        html_text = f"<html> \
<head> \
</head> \
<body> \
...
# --- Login dialog for CVAT credentials (GUI) ---
...
# --- Main toolkit for annotation integration (CVAT API/manual) ---
class AnnotationToolkit:
    def __init__(self, opt, is_gui, is_two_d=True, text_browser_signal=None):
        ...
        # Handle CVAT API login if required
        if self.use_api and self.tool_name == "cvat":
            if is_gui:
                self.print_msg(
                    "Please enter your CVAT.ai account credential on the opened window."
                )
            ...
            self.configuration = Configuration(
                host="https://app.cvat.ai", username=self.user, password=self.password
            )
            try:
                _ = ApiClient(self.configuration)
            except:
    ...
    # --- Retry wrapper for API calls, with error handling and upload support ---
        ...
        for i in range(max_try):
            try:
                if is_upload_annotation:
                    task_annotations_update_request = TaskAnnotationsUpdateRequest(
                        annotation_file=open(
                            os.path.join(self.checkpoint_dir, "Subset.zip"), "rb"
                        )
                    )
                    kwargs["task_annotations_update_request"] = (
                ...
                if response.status in [200, 201, 202] and response.data != b"":
                    ...
                    self.print_msg(
                        f"Api call was not successful! {response.data}. Trying again ..."
                    )
            except Exception as e:
                self.print_msg(traceback.format_exc())
                self.print_msg(
                    f"Exception when calling CVAT.ai TasksApi {e}\n. Please try again or use manual integration with CVAT.ai."
                )
        self.print_msg(
            "Couldn't make the api call after maximum tries. Please try again or use manual integration with CVAT.ai."
        )
        exit(1)

    # --- Create a new annotation task or reuse existing one on CVAT ---
    def create_task(self):
        if self.use_api:
            if self.tool_name == "cvat":
                with ApiClient(self.configuration) as api_client:
                    data, _ = self.try_func(
                        api_client.tasks_api.list, name=self.opt.exp_name
                    )
                    if len(data.results) > 0:
                        ...
                        task, _ = self.try_func(
                            api_client.tasks_api.create, task_write_request=task_spec
                        )
                        self.task_id = task.id
                        self.print_msg(
                            f"Annotation task with id:{self.task_id} is created on CVAT.ai."
                        )
                        root_files = self.opt.data_dir
                        if self.opt.data_format == "kitti":
                            root_files = os.path.join(
                                root_files,
                                "image_2" if self.is_two_d else "velodyne_reduced",
                            )
            ...
            if self.tool_name == "cvat":
                if self.is_gui:
                    instruction_ui = InstructionUI(
                        self, self.get_messages("labeling_tool")
                    )
                    instruction_ui.exec()
                else:
    ...
    # --- Upload pre-annotations to CVAT task (API) ---
            ...
            if self.tool_name == "cvat":
                ...
                with ApiClient(self.configuration) as api_client:
                    self.try_func(
                        ...
                        is_upload_annotation=True,
                        id=self.task_id,
                        format="KITTI 1.0" if self.is_two_d else "Kitti Raw Format 1.0",
                        _content_type="multipart/form-data",
                    )
                self.print_msg("The subset pre-annotation is uploaded to the task.")

    # --- Guide user to correct/verify annotations (API/manual) ---
        ...
        if self.use_api:
            if self.tool_name == "cvat":
                ...
                with ApiClient(self.configuration) as api_client:
                    while True:
                        data, _ = self.try_func(
                            api_client.jobs_api.list, task_id=self.task_id
                        )
                        if len(data.results) > 0:
                            break
                        self.print_msg(
                            "Wait for a few minutes untill the task is ready for annotation on CVAT.ai server ..."
                        )
                        sleep(60)

                    job_id = data.results[0].id
                    url = f"https://app.cvat.ai/tasks/{self.task_id}/jobs/{job_id}"
                    # Launch or reuse Selenium driver for browser automation
                    if self.driver is None:
                        ...
                        self.driver.find_element(
                            By.XPATH,
                            '//*[@id="root"]/section/section/main/div/div[2]/div/div/div/form/div[3]/div/div/div/button',
                        ).click()
                    else:
                        self.driver.get(url)
                        ...
                        if self.is_gui:
                            if ret == QMessageBox.Yes:
                                while True:
                                    ret = show_msgbox(
                                        self,
                                        "Have you saved the project on CVAT.ai?",
                                        "yes/no",
                                        "info",
                                        self.is_gui,
                                    )
                    ...
                    data, response = self.try_func(
                        api_client.tasks_api.retrieve_annotations,
                        id=self.task_id,
                        format="KITTI 1.0" if self.is_two_d else "Kitti Raw Format 1.0",
                        filename="subset.zip",
                        action="download",
                    ...
                    with open(
                        os.path.join(self.checkpoint_dir, "Refined_Subset.zip"), "wb"
                    ) as output_file:
            ...
            if self.is_gui:
                instruction_ui = InstructionUI(
                    self, self.get_messages("label_refinement")
                )
                instruction_ui.exec()
            else:
    ...
    # --- Destroy all annotations for the current task (API) ---
    def destroy_annotation(self):
        if self.use_api and self.tool_name == "cvat":
            with ApiClient(self.configuration) as api_client:
                api_client.tasks_api.destroy_annotations(id=self.task_id)

    # --- Delete the current task from CVAT (API) ---
    ...
    # --- Get user-facing messages for instructions and prompts ---
    def get_messages(self, key):
        ...
        if self.tool_name == "cvat":
            ...
            refined_ann_filename = (
                "Refined_Subset.zip"
                if self.opt.data_format == "kitti"
                else "Refined_Subset_OL_annotations.json"
            )
    ...
    def check_annotation(self):
        ...
        if not file_exist(subset_ann_path):
            while True:
                ret = show_msgbox(
                    self,
                    f"{subset_ann_path} has not been found. Please export the refined annotations as instructed and press ok once done.",
                    "OK",
                    "error",
                    self.is_gui,
                )
