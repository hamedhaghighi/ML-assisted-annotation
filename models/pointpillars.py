...
from ops import Voxelization, nms_cuda, nms_normal_gpu
from models.utils import limit_period, iou2d, iou3d_camera, iou_bev, get_score_thresholds
...
class PointPillars(nn.Module):
    ...
    def get_output(self, data_dict, classes_to_labels=None):
        ...
        batched_difficulty = data_dict['batched_difficulty']
        ret_results = []
        batch_results = self.__call__(batched_pts=batched_pts, 
                                mode='val',
                                batched_gt_bboxes=batched_gt_bboxes, 
                                batched_gt_labels=batched_labels)
        for res in batch_results:
            keep_ind = [i for i,l in enumerate(res['labels']) if classes_to_labels[l] != -1]
            if len(keep_ind) > 0:
                ret_results.append({k:v[keep_ind] for k, v in res.items()})
            else:
                ret_results.append(None)
            
        return ret_results


    def evaluate(self, det_results, gt_results, CLASSES):
    ...
    def m_train(self, data_dict, optimizer, scheduler, labels_to_classes, vis, len_dataloader, epoch, batch_i, nclasses, print_msg_fn, total_steps):
        if self.is_cuda:
            for key in data_dict:
                for j, item in enumerate(data_dict[key]):
                    if torch.is_tensor(item):
        ...
        bbox_cls_pred, bbox_pred, bbox_dir_cls_pred, anchor_target_dict = \
            self.__call__(batched_pts=batched_pts, 
                            mode='train',
                            batched_gt_bboxes=batched_gt_bboxes, 
                            batched_gt_labels=batched_labels)
        ...
        if labels_to_classes is not None:
            ...
            batched_bbox_labels[has_annotation_ind] = \
                tensor_labels_to_class[batched_bbox_labels[has_annotation_ind].long()].to(batched_bbox_labels)
        ...
        loss_dict = self.Loss(bbox_cls_pred=bbox_cls_pred,
                                bbox_pred=bbox_pred,
