...
from models.utils import (get_score_thresholds, iou2d, iou3d_camera, iou_bev,
                          limit_period)
    ...
    def get_output(self, data_dict, classes_to_labels=None):
        ...
        for res in batch_results: 
            ...
            if len(keep_ind) > 0:
                ...
                labels = np.zeros(1) + -1
                scores = np.zeros(1)
                ret_results.append({'lidar_bboxes':bboxes, 'labels':labels, 'scores':scores})
        return ret_results
        ...
        for e_ind, eval_type in enumerate(['bbox_2d', 'bbox_bev', 'bbox_3d']):
            ...
            for cls in CLASSES:
                ...
                if eval_type == 'bbox_2d':
                    ...
                    mSimilarity = sums_similarity / np.ceil(len(score_thresholds)/4) * 100
