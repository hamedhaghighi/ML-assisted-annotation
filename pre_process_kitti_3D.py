...
def create_data_info_pkl(data_root, data_type, prefix, label=True, db=False):
    ...
    lidars_path = sorted(glob.glob(f'{data_root}/velodyne/*.*'))
    labels_path = sorted(glob.glob(f'{data_root}/label_2/*.*'))
    images_path = sorted(glob.glob(f'{data_root}/image_2/*.*' ))
    calibs_path = sorted(glob.glob(f'{data_root}/calib/*.*' ))
    ...
    for i in trange(len(lidars_path)):
        cur_info_dict={}
        img_path = images_path[i]
        lidar_path = lidars_path[i]
        calib_path = calibs_path[i]
        label_path = labels_path[i]
        id = lidar_path.split(sep)[-1].split('.')[0]
        cur_info_dict['velodyne_path'] = sep.join(lidar_path.split(sep)[-2:])
        img = cv2.imread(img_path)
        image_shape = img.shape[:2]
        cur_info_dict['image'] = {
            'image_shape': image_shape,
            'image_path': sep.join(img_path.split(sep)[-2:]), 
            'image_idx': int(id),
        }
        ...
        reduced_lidar_points = remove_outside_points(
            ...
            P2=calib_dict['P2'], 
            image_shape=image_shape)
        saved_reduced_path = os.path.join(data_root, 'velodyne_reduced')
...
def main(args):
    ...
    ##           && create database(points in gt bbox) for data augmentation
    create_data_info_pkl(data_root, 'train', prefix, db=True)
