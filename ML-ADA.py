...
from utils.utils import (
    ...
    set_device,
    show_msgbox,
    unpack_pre_annotation,
    weights_init_normal,
)

# --- Argument parsing and mode selection (GUI/CLI) ---
# Determine if the application should run in GUI or CLI mode based on command-line arguments
is_gui = None
if len(sys.argv) > 1:
    if sys.argv[1] == "--gui":
        is_gui = True
        # Import GUI-specific modules only if running in GUI mode
        ...
        is_gui = False
        inhertance_classes = ()
# If no valid mode is specified, print usage and exit
if is_gui is None:
    print("Please use --gui or --cfg options.")
    exit(1)

# --- Helper class to parse and store options ---
class MargParse:
    def __init__(self, opt):
        ...
        for k, v in opt.items():
            setattr(self, k, v)
        # Handle annotation tool options
...
# --- Main MLADA class: core logic for annotation, training, and GUI/CLI ---
class MLADA(*inhertance_classes):
    ...
    def __init__(self, opt, parent=None):
        if is_gui:
            ...
            self.setupUi(self)
            self.show()
            # Thread for subset selection in GUI
        ...
        print(opt)
        opt_dict = opt.copy()  # Keep original dict for key-based access
        opt = MargParse(opt)
        ...
        self.do_training = getattr(opt, 'do_training', True)
        ...
        self.extension = "jpeg"
        self.skip_rt = False
        model_name = getattr(opt, 'model_name', opt_dict.get('model_name', 'default'))
        exp_name = getattr(opt, 'exp_name', opt_dict.get('exp_name', 'default'))
        cfg = f"config/{model_name}.cfg"
        opt.checkpoint_dir = os.path.join(opt.checkpoint_dir, exp_name)
        resume = False
        # --- Handle experiment resuming or overwriting ---
        if os.path.exists(opt.checkpoint_dir):
            while True:
                ret = show_msgbox(
                    self,
                    f"The experiment name {exp_name} already exists. Do you want to resume annotation?\nPress yes to resume it, no to overwrite the project:",
                    "yes/no",
                    "info",
                    self.is_gui if self.is_gui is not None else False,
                )
                if self.is_gui:
        ...
        labels = getattr(opt, 'labels', opt_dict.get('labels', []))
        labels_to_classes = getattr(opt, 'labels_to_classes', opt_dict.get('labels_to_classes', []))
        #######################################################################
        ...
        data_dir = getattr(opt, 'data_dir', opt_dict.get('data_dir', ''))
        data_format = getattr(opt, 'data_format', opt_dict.get('data_format', ''))
        img_size = getattr(opt, 'img_size', opt_dict.get('img_size', 416))
        ...
        # --- Visualizer for logging and plots ---
    ...
    # --- Thread starter for GUI subset selection ---
    def start_thread(self, thread):
        if thread.isFinished:
            thread.start()

    # --- Select a subset of data for annotation (active learning) ---
    def subset_selection(self):
        ...
        if self.loop_iteration == 0 and len(self.logger.selected_filenames) > 0:
            ...
            # Confidence-based selection: pick samples with lowest mean confidence
        ...
        # Order subset indices for consistency
    ...
    # --- Visualize model predictions and/or ground truth for current sample ---
    ...
    # --- Run model to pre-annotate a subset of data ---
    ...
    # --- Evaluate model and pre-annotations on the current subset ---
    def evaluate(self, pre_annotation_list):
        self.print_msg("\nEvaluation .....\n")
        tag = "subset"
        if tag != "subset":
            pre_annotation_list = None
        labelled_sub_dataset = Image2DAnnotationDataset(
            os.path.join(
            ...
            self.opt.img_size,
            self.resize_tuple,
            img_root_dir=self.opt.data_dir,
        ...
        d = time.time() - s
        self.total_time_iter += d
        MAF = self.model.evaluate_MAF(
            data_loader,
            self.classes_to_labels,
            pre_annotation_list,
            self.progressBar if self.is_gui else None,
        )
        MAF_no_MLADA = self.model.evaluate_MAF(
            data_loader, self.classes_to_labels, [None] * len(pre_annotation_list), None
        )

        if mAP > self.val_best_mAP:
            self.val_best_mAP = mAP
            self.model.save_weights("%s/kitti_best.weights" % (self.opt.checkpoint_dir))
        ...
        with open(
            os.path.join(
                self.opt.checkpoint_dir, self.opt.query_mode + "_" + tag + "_MAF.pkl"
            ),
            "wb",
        ) as f:
        ...
        with open(
            os.path.join(
                self.opt.checkpoint_dir,
                self.opt.query_mode + "_" + tag + "_MAF_no_MLADA.pkl",
            ),
            "wb",
        ) as f:
    ...
    # --- Fine-tune the model on the current labelled data ---
    def fine_tune_thread(self, is_training=True):
        ...
        train_set = Image2DAnnotationDataset(
            total_data_dir,
            self.labels,
            self.labels_to_classes,
            self.opt.data_format,
        ...
        for epoch in range(self.opt.epochs if is_training else 1):
            ...
            self.model.freeze_parameters(
                self.training_iteration / self.total_iterations,
                freeze_backbone,
                is_training,
            )
            if is_training:
            ...
            for batch_i, (_, imgs, targets) in enumerate(dataloader):
                ...
                if self.labels_to_classes is not None:
                    ...
                    tensor_labels_to_class = set_device(
                        torch.tensor(self.labels_to_classes), self.is_cuda
                    )
                    x, y = torch.nonzero(no_annotation_ind, as_tuple=True)
                    targets[x, y, 0] = tensor_labels_to_class[
                        targets[x, y, 0].long()
                    ].double()
                ...
                if is_training:
                    ...
                    self.vis.plot("total_loss", loss.item(), self.total_steps)
            ...
            losses_list = (
                None
                if is_training
                else {k: np.array(v).mean() for k, v in losses_list.items()}
            )
        d = time.time() - s
    ...
    # --- Stop training if performance threshold is reached ---
    def check_performance(self):
        if self.val_best_mAP > self.opt.performance_thres:
            self.do_training = False

    # --- Export pre-annotations to disk in KITTI format ---
    def export_annotation(self, dataset, pre_annotation_list):
        ...
        total_pre_label_dir = os.path.join(
            self.opt.checkpoint_dir, "total", "pre_label_2"
        )
        os.makedirs(pre_label_dir, exist_ok=True)
        os.makedirs(total_pre_label_dir, exist_ok=True)
        frames = []
        for ind, ((img_path, _, _), pre_annotation) in enumerate(
            ...
            with open(os.path.join(pre_label_dir, label_filename), "w") as f:
                if pre_annotation is not None:
                    ...
                    for pre_ann in pre_annotation:
                        line = ["-1.0"] * 15
                        line[1], line[2] = "0.0", "3"
                        c, x1, y1, x2, y2 = unpack_pre_annotation(
                            pre_ann, pad_x, pad_y, unpad_h, unpad_w, h, w, self.labels
                        )
                        x1, y1, x2, y2 = (
                            round(x1, 2),
                            round(y1, 2),
                        ...
                        line[0], line[4], line[5], line[6], line[7] = (
                            c,
                            str(x1),
                            str(y1),
                            str(x2),
                ...
                # else:
                line = ["-1.0"] * 15
                line[0], line[1], line[2] = "DontCare", "0.0", "3"
                f.write(" ".join(line) + "\n")
    ...
    # --- Copy images and labels for manual refinement (GUI) ---
    def refine_pre_labels(self, dataset):
        ...
        if self.opt.data_format == "kitti":
            for img_path, _, _ in dataset:
                ...
                label_path = img_path.replace("image", "label").replace(
                    self.extension, "txt"
                )
    ...
    # --- Main annotation loop for one subset (active learning cycle) ---
    def annotate_next_subset(self):
        ...
        self.print_msg(
            f"total time for iteration {self.loop_iteration} is {self.total_time_iter}"
        )
    ...
    # --- GUI: handle window close event ---
    def closeEvent(self, event):
        self.exit_(0)

# --- GUI entry point: start MLADA from config window ---
def start_MLADA(config_window):
    mainwindow = MLADA(opt=config_window.get_all_variables())

# --- Main entry point: handle CLI and GUI startup ---
    ...
    if is_gui:
        ...
        if os.path.exists(cfg_file_path):
            ...
            if (
                ...
                or not os.path.exists(os.path.join(opt["data_dir"], "label_2"))
            ):
                print(
                    f"The path {opt.data_dir} does not contain image_2 or label_2 folders. Please check the structure of kitti dataset."
                )
                exit(1)
