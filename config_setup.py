...
# --- Advanced configuration dialog for extra options ---
...
# --- Main configuration dialog for experiment setup ---
class ConfigUI(QDialog, Ui_Dialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setupUi(self)
        # Initialize lists and default values for configuration options
        ...
        self.advanced_conf_var_dict = {
            "epochs": 5,
            "batch_size": 32,
            "subset_size": 25,
            "performance_thres": 0.5,
            "query_mode": "random",
        }
        self.conf_var_names = [
            "exp_name",
        ...
        # var_dict holds the current value and flag for each config variable
        self.var_dict = {
            name: {"val": 0, "flag": False} for name in self.conf_var_names
        }
        ...
        self.performance_thresh.setValue(
            int(self.advanced_conf_var_dict["performance_thres"] * 100)
        )
        ...
        self.use_cuda.stateChanged.connect(
            lambda: self.set_var("use_cuda", self.use_cuda.isChecked())
        )
        self.data_format.activated.connect(
            lambda: self.set_var("data_format", self.data_format.currentText())
        )

        self.epochs.valueChanged.connect(
            lambda: self.set_var("epochs", self.epochs.value())
        )
        self.batch_size.valueChanged.connect(
            lambda: self.set_var("batch_size", self.batch_size.value())
        )
        self.subset_size.valueChanged.connect(
            lambda: self.set_var("subset_size", self.subset_size.value())
        )
        self.performance_thresh.valueChanged.connect(
            lambda: self.set_var(
                "performance_thres", self.performance_thresh.value() / 100.0
            )
        )
        self.query_mode.activated.connect(
            lambda: self.set_var("query_mode", self.query_mode.currentText())
        )
        self.import_config.clicked.connect(self.read_cfg)
        self.first_ok.clicked.connect(self.save_conf)
        self.cvat_api_button.clicked.connect(
            lambda: self.set_var("annotation_tool", "cvat_api")
        )
        self.cvat_manual_button.clicked.connect(
            lambda: self.set_var("annotation_tool", "cvat_manual")
        )
        self.general_button.clicked.connect(
            lambda: self.set_var("annotation_tool", "general")
        )
    ...
    def set_var(self, name, val):
        ...
        # Enable OK button only if all required fields are set
    ...
    # --- Update label-to-class mapping when model changes ---
    ...
    # --- Save experiment name and update config ---
    ...
    def open_file_dialog(self):
        if not self.var_dict["data_format"]["flag"]:
            show_msgbox(
                self, "Please select data format before browsing the directory."
            )
        else:
            data_dir = str(QFileDialog.getExistingDirectory(self, "Select Directory"))
            if (
                self.var_dict["data_format"]["val"] == "kitti"
                and not os.path.exists(os.path.join(data_dir, "image_2"))
                or not os.path.exists(os.path.join(data_dir, "label_2"))
            ):
                show_msgbox(
                    self,
                    f"The path {data_dir} does not contain image_2 or label_2 folders. Please check the structure of kitti dataset.",
                )
            else:
                self.data_dir.setText(data_dir)
                self.set_var("data_dir", data_dir)

    # --- Add label and label-to-class mapping to the list ---
    ...
    # --- Clear all label/class mappings ---
    ...
    # --- Get all config variables as a dict ---
    def get_all_variables(self):
        return {k: v["val"] for k, v in self.var_dict.items()}

    # --- Import config from a JSON file ---
    def read_cfg(self):
        cfg_file_path, _ = QFileDialog.getOpenFileName(
            self, "Select Config File", filter="*.json"
        )
